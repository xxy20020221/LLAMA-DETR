...
class LLAMADETR(LlavaLlamaForCausalLM):
    def __init__(self,
                 llava,
                 transformer,
                 num_classes,
        ...
        # * 这个proj不知道需不需要，如果已经对齐了就不要了
        # self.input_proj = nn.Linear(llava.dim,hidden_dim)
...
def build_LLAMADETR(args):
    num_classes = 20 if args.dataset_file != 'coco' else 91
    if args.dataset_file == "coco_panoptic":
        num_classes = 250
    device = torch.device(args.device)
    # build llava model
    llava = build_llava(args)
    # build llama decoder
    if args.use_llama_decoder:
        transformer = build_llama_decoder(args)
    else:
